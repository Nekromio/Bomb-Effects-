...
int GetCSGame()
{
	...
	else if (GetFeatureStatus(FeatureType_Native, "GetEngineVersion") == FeatureStatus_Available) 
	{ 
		switch (GetEngineVersion())
		{ 
...
{
	if(IsValidEntity(iBall) || IsValidEntity(iBall_2) || IsValidEntity(iBall_3))
	{
		/*if(iBall > 0) AcceptEntityInput(iBall, "Kill");
		if(iBall_2 > 0) AcceptEntityInput(iBall_2, "Kill");
		if(iBall_3 > 0) AcceptEntityInput(iBall_3, "Kill");*/
		//KillTimer(hTimerEffect, true);
	}
	return Plugin_Stop;
}